...
class _UploadMeterReadingPageState extends State<UploadMeterReadingPage> {
  ...
  void _submitReading() {
    ...
    if (currentReading < lastMonthReading) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Current reading cannot be less than the previous reading.')),
      );
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Reading Submitted. Bill: \$${billAmount.toStringAsFixed(2)}')),
    );
